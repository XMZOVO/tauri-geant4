...
const detectorList = ref([
  { name: "碘化钠探测器", percent: 40, description: "Sodium Iodide Detector" },
  {
    name: "溴化澜探测器",
    percent: 100,
      ...
      <div class="flex h-2/5 space-x-5">
        ...
        <div class="card w-1/2 bg-base-100 shadow-xl">
          <div class="card-body">
            ...
            <!-- item -->
            <ul
              class="menu h-full w-full justify-between rounded-xl bg-base-100"
            >
              <li v-for="(item, index) in detectorList" class="h-full">
                <a :class="{ [`active`]: index == activeDetector }">{{
                  item.name
                }}</a>
              </li>
            </ul>
