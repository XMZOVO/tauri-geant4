...
const caibrateEfficiency = async () => {
  alert(tableContent.value[0].energy);
  return;
  //效率刻度系数
  ...
  for (let i = 0; i < effList.length; i++) {
    tableContent.value[i][2] = effList[i];
  }
};
...
<template>
  ...
  <div class="h-1/2 overflow-x-auto px-10 py-6">
      ...
      <div class="dropdown">
        ...
        >
          <div class="card-body">
            ...
            <div class="btn" @click="caibrateEfficiency">刻度</div>
          </div>
        </div>
      </div>
    </div>
    <table class="table-compact table w-full">
      ...
      <tbody>
        <tr v-for="item in tableContent" :key="item.number">
          <th v-for="data in item" :key="item.number">
            <input
              type="text"
