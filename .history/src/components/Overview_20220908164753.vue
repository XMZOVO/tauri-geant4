...
const tableContent = ref([
  { number: 1, energy: 1, efficiency: 0, fit: 0, delta: 0 },
  { number: 2, energy: 1, efficiency: 0, fit: 0, delta: 0 },
  { number: 3, energy: 1, efficiency: 0, fit: 0, delta: 0 },
  { number: 4, energy: 1, efficiency: 0, fit: 0, delta: 0 },
  { number: 5, energy: 1, efficiency: 0, fit: 0, delta: 0 },
...
const caibrateEfficiency = async () => {
  ...
  //绘制曲线
  const efficiencyData = generateLineFit(energyList);
  option = {
    xAxis: {
      type: "category",
      data: energyList,
...
const generateLineFit = (energyList: number[]): number[] => {
  let efficiencyList: number[] = [];
  for (let i = 0; i < energyList.length; i++) {
    let efficiency: number = Math.exp(
      a1.value + a2.value * Math.log(efficiencyList[i])
    );
...
<template>
  ...
  <div class="h-1/2 overflow-x-auto px-10 py-6">
    <div class="flex items-center gap-3 pb-5">
      ...
      <div class="dropdown">
        ...
        >
          <div class="card-body">
            <div class="felx flex-col space-y-3">
  ...
  <!-- Chart -->
  <p class="px-10 text-lg font-bold">能谱</p>

  <!--  -->
  <div class="flex h-1/2 items-center justify-center overflow-x-visible pb-6">
