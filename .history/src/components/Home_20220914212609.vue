...
let detectorSrc = ref(
  JSRootPath.value +
    "index.htm?nobrowser&file=http://localhost:8080/file/G4Backend/build/rootFiles/wtest.root&item=Default;1&opt=ssao;transp60;zoomFACTOR=150"
);
...
<template>
  ...
  <div class="scrollbar-hide flex h-full w-full overflow-hidden">
    <div
      class="flex w-2/3 flex-col justify-between space-y-6 border-r border-base-300 py-6 px-16"
    >
      <!-- 标语 -->
      ...
      <div class="card h-2/6 w-full shadow-xl">
      ...
      <!-- 底部双卡片 -->
      <div class="flex h-3/6 space-x-5">
        <!-- 1 -->
        <div class="card w-1/2 overflow-visible bg-base-100 shadow-xl">
          <div class="card-body gap-3">
            <p class="text-sm font-bold">探测器模版选择</p>
            <!-- item -->
            <div class="flex h-full flex-col justify-between">
              <div
                v-for="(item, index) in detectorList"
                class="flex cursor-pointer items-center justify-between rounded-xl py-3 px-3 hover:bg-opacity-20"
                :class="{
                  [` hover:bg-orange-400`]: index == 0,
                  [` hover:bg-red-400`]: index == 1,
                  [` hover:bg-cyan-500`]: index == 2,
                }"
              >
                <div class="flex items-center gap-2">
                  <img
                    :src="`/src/assets/${item.img}`"
                    class="h-10 w-10 rounded-xl p-1 2xl:h-16 2xl:w-16"
                    :class="{
                  ...
                  <div class="">
                    ...
                    <p class="hidden truncate text-xs opacity-60 2xl:visible">
                      {{ item.description }}
                    </p>
                    <div
                      class="badge badge-xs p-2"
                      :class="{
                        [` badge-primary`]: index == 0,
                        [`badge-secondary`]: index == 1 || index == 2,
                      }"
                    >
                      {{ index == 0 ? "已实现" : "未实现" }}
                    </div>
            ...
            <div class="dropdown-right dropdown-end dropdown">
              <label tabindex="0" class="btn w-full">更多模版</label>
              <div
                tabindex="0"
                class="card dropdown-content card-compact w-64 bg-base-100 p-2 shadow"
              >
                <div class="card-body">
                  <div class="flex w-full items-center justify-center">
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <span class="font-semibold">点击或拖放GDML文件</span>
                        </p>
