...
onMounted(async () => {
  ...
  series.columns.template.events.on("click", function (ev) {
    if (alertVisible.value == true) {
      return;
    }
    let countItem: any = ev.target.dataItem!.dataContext;
...
const generateData = (data: string[], energy: string[]) => {
  ...
  for (let i = 0; i < data.length; i++) {
    outData.push({
      channel: parseFloat(energy[i]).toFixed(4).toString(),
      count: parseFloat(data[i]),
    });
...
<template>
  <div
    class="alert alert-success absolute top-10 right-10 z-50 w-1/4 align-middle opacity-0 shadow-2xl transition-opacity duration-300"
    :class="{ [`opacity-100`]: alertVisible }"
  >
    <div>
      <svg
  ...
  <div class="flex h-full flex-col overflow-hidden">
    <!-- table1 -->
    <div class="relative h-1/2 overflow-x-auto px-10 py-6">
      ...
      <div class="btn-group absolute right-10">
        <button class="btn btn-sm">1</button>
        <button class="btn btn-active btn-sm">2</button>
        <button class="btn btn-sm">3</button>
        <button class="btn btn-sm">4</button>
      </div>
    </div>
    <!-- Chart -->
    <div class="h-1/2 flex-col space-y-5 px-10">
      <!-- tab -->
      <div class="tabs">
        <a
          @click="toSpec"
