...
let detectorSrc = ref(
  JSRootPath +
    "index.htm?nobrowser&file=http://localhost:8080/file/wtest.root&item=Default;1&opt=ssao;transp60"
);
...
const executeCalculate = async () => {
  ...
  const response = await fetch("http://localhost:8080/g4", {
    method: "POST",
    query: {
      ch: detectorProperties.value[0].value,
      cr: detectorProperties.value[1].value,
      rtt: detectorProperties.value[2].value,
      num: paticleParameters.value[0].value,
    },
  }).then((res) => {
    btnWait.value = "";
    console.log(res);
