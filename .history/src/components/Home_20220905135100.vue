...
import { useIsActiveIndexStore } from "../stores/isActiveIndex";
import { exit } from "@tauri-apps/api/process";

const isActiveIndex = useIsActiveIndexStore();

const router = useRouter();
const greetMsg = ref("");
const name = ref("");
const parametersNames = ref([
  { name: "钻井液性能", checked: true },
  { name: "节流压井", checked: false },
]);
const selectedName = ref("钻井液性能");
const drillingFluidProperties = ref([
  { name: "钻井液密度", value: 2.32, unit: "g/cm³" },
  { name: "塑性粘度", value: 40, unit: "mPa.s" },
  { name: "动切力", value: 10, unit: "Pa" },
  { name: "界面张力", value: 0.04, unit: "Pa/m" },
]);
const throttlingProperties = ref([
  { name: "关井立压", value: 2.01, unit: "MPa" },
  { name: "关井套压", value: 4.87, unit: "MPa" },
  { name: "井口温度", value: 20, unit: "Pa" },
  { name: "温度梯度", value: 0.02, unit: "°C" },
  { name: "压并排量", value: 10, unit: "L/s" },
]);
...
const otherParameters = ref([
  { name: "指定井深处流型", value: 10 },
  { name: "初始溢流物高度", value: 1086.46 },
]);
const btnWait = ref("");
  ...
  <div class="flex h-full w-full">
    <div class="w-2/3 space-y-12 border-r border-base-300 py-6 px-16">
      <div class="h-auto">
        ...
        <span class="block text-3xl font-bold">Welcome back &#128517;</span>
      </div>
        ...
        <div class="card-body justify-between">
          ...
          >
            <div
              ...
              v-for="item in drillingFluidProperties"
            >
            ...
            <div
              ...
              v-for="item in throttlingProperties"
            >
              <div class="stat-title">{{ item.name }}</div>
              <div class="stat-value">{{ item.value }}</div>
              <div class="stat-desc">{{ item.unit }}</div>
        ...
        <div class="card w-1/2 bg-base-100 shadow-xl">
          <div class="card-body">
            <div class="card-actions justify-end">
              <div class="btn" @click="exitApp">Exit</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="flex-col space-y-5">
              ...
              <div class="dropdown-left dropdown w-full">
                ...
                >
                  <img src="../assets/icon.png" class="h-14 w-14 rounded-xl" />
                  <div class="flex items-center">
                    <p class="text-sm font-bold">钻井液性能参数</p>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
              ...
              <div class="dropdown-left dropdown w-full">
                ...
                >
                  <img src="../assets/icon.png" class="h-14 w-14 rounded-xl" />
                  <div class="flex items-center">
                    <p class="text-sm font-bold">节流压井参数</p>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
      ...
      <div class="flex space-x-5">
        <div class="card w-full bg-base-100 shadow-xl">
          <div class="card-body space-y-5">
            <p class="text-md font-bold">执行计算</p>
            <div>
              <div v-for="item in otherParameters" class="form-control">
                <label class="label">
                  <span class="label-text">{{ item.name }}</span>
                </label>
                <label class="input-group">
