...
let guiInput: InputBindingApi<unknown, unknown>[];
let isLoaded = false;
...
const createLight = () => {
  ...
  const guiDirlightInput = gui.pane.addInput(dirLight, "intensity", {
    min: 0,
    max: 1,
  });
  guiInput.push(guiDirlightInput);
...
const createAxesHelper = () => {
  ...
  const guiAxeInput = gui.pane.addInput(axesHelper, "visible", {
    label: "AxesHelper",
  });
  guiInput.push(guiAxeInput);
};
