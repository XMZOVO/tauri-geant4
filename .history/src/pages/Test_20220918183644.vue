...
const elSizeInfo = useElementBounding(ThreeDom);
...
const createCamera = () => {
  camera = new THREE.PerspectiveCamera(
    75,
    elSizeInfo.width.value / elSizeInfo.height.value,
    0.1,
    1e10
  );
...
const onPointerMove = (event: PointerEvent) => {
  pointer.x = (event.clientX / elSizeInfo.width.value) * 2 - 1;
  pointer.y = -(event.clientY / elSizeInfo.height.value) * 2 + 1;
};
...
onMounted(() => {
  ...
  window.addEventListener("resize", () => {
    camera.aspect = elSizeInfo.width.value / elSizeInfo.height.value;
    camera.updateProjectionMatrix();
    renderer.setSize(elSizeInfo.width.value, elSizeInfo.height.value);
